...
float noise2D(float x, float y, long seed){
    ...
    float xFrac = x - xInt;
    float yFrac = y - yInt;
    int s = noise2(xInt, yInt, seed);
    int t = noise2(xInt + 1, yInt, seed);
    int u = noise2(xInt, yInt + 1, seed);
    int v = noise2(xInt + 1, yInt + 1, seed);
    float low = smoothInterpolate(s, t, xFrac);
    float high = smoothInterpolate(u, v, xFrac);
    return smoothInterpolate(low, high, yFrac);
}
